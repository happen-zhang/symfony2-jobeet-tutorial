...
就像在*indexAction()*中一样，*IbwJobeetBundle repository*是用来检索一个指定的*Job*数据，但这次使用的是*find()*方法。*find()*方法的参数是能一个够唯一标识*Job*的标识符，即**主键**。在下面的内容中我们会探索*showAction()*是怎么样得到*Job*数据的主键*$id*的值。
